...
from accounts.serializers import UserRegistrationSerializer, LoginSerializer,PasswordResetRequestSerializer, SetNewPasswordSerializer
...
class ResgisterUserView(GenericAPIView):
...
class VerifyUserEmailView(GenericAPIView):
    def post(self, request):
        try:
            ...
            return Response({
                "message": "passcode is invalid or user already verified"
...
class TestingAuthenticatedReq(GenericAPIView):
    ...
    def get(self, request):
        data = {
            'msg': "hello there here is you profile"
        }
...
class PasswordResetRequestView(GenericAPIView):
    ...
    def post(self, request):
        serializer = self.serializer_class(data=request.data, context={'request':request})
        serializer.is_valid(raise_exception=True)
        return Response({'message': 'You password request link is sent to your email'}, status=status.HTTP_200_OK)
...
class PasswordResetConfirm(GenericAPIView):
    def get(self, request, uidb64, token):
        try:
            ...
            return Response({"message": "token is invalid or expires"}, status=status.HTTP_401_UNAUTHORIZED)
